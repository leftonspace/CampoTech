...
describe('CUIT Validation', () => {
  describe('validateCUIT', () => {
    describe('Valid CUITs', () => {
      ...
      it('should return cleaned digits in result', () => {
        const result = validateCUIT('20-27894562-6');
        expect(result.digits).toBe('20278945626');
      });
    });
    ...
    describe('Invalid CUITs', () => {
      ...
      it('should reject CUIT with invalid verification digit', () => {
        // 20-27894562-6 is valid, so 20-27894562-5 should be invalid
    ...
    describe('Edge Cases', () => {
      it('should handle CUIT with all zeros in middle (invalid checksum)', () => {
        const result = validateCUIT('20-00000000-0');
        // This would need to be checked against actual checksum
        expect(result.valid).toBe(true); // 20-00000000-0 actually has valid checksum!
      });

      it('should strip non-digit characters', () => {
        const result = validateCUIT('20.278.945.626');
        expect(result.valid).toBe(true);
        expect(result.formatted).toBe('20-27894562-6');
      });
    });
  ...
  describe('formatCUIT', () => {
    it('should format 11 digits correctly', () => {
      expect(formatCUIT('20278945626')).toBe('20-27894562-6');
    });

    it('should handle already formatted CUIT', () => {
      expect(formatCUIT('20-27894562-6')).toBe('20-27894562-6');
    });
    ...
    it('should truncate input longer than 11 digits', () => {
      expect(formatCUIT('202789456261234')).toBe('20-27894562-6');
    });
