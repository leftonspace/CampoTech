...
describe('Tier Limits Configuration', () => {
  describe('Pricing', () => {
    ...
    it('should have correct pricing for BASICO tier ($25)', () => {
      expect(TIER_LIMITS.BASICO.priceUsd).toBe(25);
      expect(TIER_LIMITS.BASICO.priceDisplay).toBe('$25/mes');
    });
    ...
    it('should have correct pricing for EMPRESARIAL tier ($120)', () => {
      expect(TIER_LIMITS.EMPRESARIAL.priceUsd).toBe(120);
      expect(TIER_LIMITS.EMPRESARIAL.priceDisplay).toBe('$120/mes');
    });
  });
  ...
  describe('User Limits', () => {
    ...
    it('BASICO tier should have 1 user max', () => {
      expect(TIER_LIMITS.BASICO.maxUsers).toBe(1);
    });
    ...
    it('EMPRESARIAL tier should have unlimited users', () => {
      expect(TIER_LIMITS.EMPRESARIAL.maxUsers).toBe(Number.MAX_SAFE_INTEGER);
    });
  });
  ...
  describe('Jobs Per Month Limits', () => {
    ...
    it('BASICO tier should have 50 jobs per month', () => {
      expect(TIER_LIMITS.BASICO.maxJobsPerMonth).toBe(50);
    });
    ...
    it('EMPRESARIAL tier should have unlimited jobs', () => {
      expect(TIER_LIMITS.EMPRESARIAL.maxJobsPerMonth).toBe(Number.MAX_SAFE_INTEGER);
    });
  });
  ...
  describe('WhatsApp AI Limits', () => {
    ...
    it('BASICO tier should have 0 WhatsApp AI messages (manual only)', () => {
      expect(TIER_LIMITS.BASICO.maxWhatsAppMessagesPerMonth).toBe(0);
    });
    ...
    it('EMPRESARIAL tier should have unlimited WhatsApp AI', () => {
      expect(TIER_LIMITS.EMPRESARIAL.maxWhatsAppMessagesPerMonth).toBe(Number.MAX_SAFE_INTEGER);
    });
  });
});
...
describe('Tier Configurations', () => {
  ...
  it('BASICO should be named "Inicial"', () => {
    const basicoTier = TIER_CONFIGS.find(t => t.id === 'BASICO');
    expect(basicoTier?.name).toBe('Inicial');
  });
});
...
describe('Helper Functions', () => {
  describe('getTierLimits', () => {
    it('should return correct limits for each tier', () => {
      ...
      expect(getTierLimits('BASICO').priceUsd).toBe(25);
      expect(getTierLimits('PROFESIONAL').maxJobsPerMonth).toBe(200);
      expect(getTierLimits('EMPRESARIAL').maxUsers).toBe(Number.MAX_SAFE_INTEGER);
    });
  ...
  describe('getLimit', () => {
    it('should return correct limit values', () => {
      ...
      expect(getLimit('BASICO', 'jobs_monthly')).toBe(50);
  ...
  describe('isLimitExceeded', () => {
    ...
    it('should never exceed for unlimited tiers', () => {
      expect(isLimitExceeded('EMPRESARIAL', 'users', 1000, 1000)).toBe(false);
      expect(isLimitExceeded('EMPRESARIAL', 'jobs_monthly', 999999, 1)).toBe(false);
    });
  });
  ...
  describe('getTierOrder', () => {
    it('should return correct order values', () => {
      ...
      expect(getTierOrder('BASICO')).toBe(1);
      expect(getTierOrder('PROFESIONAL')).toBe(2);
      expect(getTierOrder('EMPRESARIAL')).toBe(3);
    });
  });

  describe('isTierHigher', () => {
    it('should correctly compare tiers', () => {
      expect(isTierHigher('BASICO', 'FREE')).toBe(true);
      expect(isTierHigher('EMPRESARIAL', 'PROFESIONAL')).toBe(true);
      expect(isTierHigher('FREE', 'BASICO')).toBe(false);
      expect(isTierHigher('BASICO', 'BASICO')).toBe(false);
    });
  });
  ...
  describe('getMinimumTierForLimit', () => {
    it('should return minimum tier for user count', () => {
      ...
      expect(getMinimumTierForLimit('users', 100)).toBe('EMPRESARIAL');
    });

    it('should return minimum tier for jobs per month', () => {
      expect(getMinimumTierForLimit('jobs_monthly', 30)).toBe('FREE');
      expect(getMinimumTierForLimit('jobs_monthly', 50)).toBe('BASICO');
      expect(getMinimumTierForLimit('jobs_monthly', 150)).toBe('PROFESIONAL');
      expect(getMinimumTierForLimit('jobs_monthly', 500)).toBe('EMPRESARIAL');
    });
  });
});
