 ...
 * Record job creation
 */
export async function recordJobCreated(orgId: string): Promise<void> {
  const client = getRedis();
  if (!client) return;
 ...
 */
export async function recordInvoiceGenerated(orgId: string): Promise<void> {
  const client = getRedis();
  if (!client) return;
 ...
 */
export async function recordAIConversation(orgId: string): Promise<void> {
  const client = getRedis();
  if (!client) return;
