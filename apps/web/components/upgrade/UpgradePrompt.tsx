...
export function UsageWarning({
  ...
  return (
    <div
      className={`px-3 py-2 rounded-lg text-sm flex items-center justify-between ${isOver ? 'bg-danger-50 text-danger-700' : 'bg-amber-50 text-amber-700'
        }`}
    >
      ...
      <Link
        href="/dashboard/settings/billing"
        className={`text-xs font-medium hover:underline ${isOver ? 'text-danger-800' : 'text-amber-800'
          }`}
      >
        Ampliar
      </Link>
